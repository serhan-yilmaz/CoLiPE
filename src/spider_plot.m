...
horzcenterall = false;
axesPostfix = cell(num_data_points, 1);
...
if numvarargs > 1
    ...
    for ii = 1:length(name_arguments)
        ...
        switch lower(name_arguments{ii})
            ...
            case 'axespostfix'
                axesPostfix = value_arguments{ii};
            otherwise
...
for ii = 1:length(theta)-1
    ...
    [x_axes, y_axes] = pol2cart(theta(ii), rho(rho >= 0));
...
for ii = 1:theta_end_index
    ...
    for jj = 2:length(rho)
        ...
        if(jj >= 3 && log10labeling(ii))
            exponent = floor(axes_value);
            remainder = 10.^(axes_value - exponent);
            remainder = round(remainder, axes_precision);
            value = 10^(axes_value);
            if((value >= 1000) || (value < 1))
                if(remainder == 1)
                    text_str = sprintf('10^{%s}', num2str(exponent));
                else
                    text_str = sprintf('%s\\times10^{%s}', num2str(remainder), num2str(exponent));
                end
            else
%                 val = remainder * 10.^exponent;
%                 text_str = sprintf('%d', round(val));
                if(value >= 10)
                    text_str = sprintf('%d', round(10^axes_value, -1));
                else
                    text_str = sprintf('%d', round(10^axes_value));
                end
            end
        else
%             text_str = sprintf(sprintf('%%.%if', axes_precision+1), axes_value);
            text_str = sprintf('%s', num2str(round(axes_value, axes_precision+1)));
        end
        
        text_str = [text_str, axesPostfix{ii}];
